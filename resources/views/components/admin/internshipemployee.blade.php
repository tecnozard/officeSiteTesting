...
<body ng-controller="InternshipController">
    @extends('components.admin.adminnavbar')

    @section('content')
    <div class="container">
        <h2 class="animate__animated animate__fadeInDown">Internship Submissions</h2>
            ...
            <table class="table table-striped table-hover">
                ...
                <tbody>
                    <tr ng-repeat="internship in internships">
                        ...
                        <td>@{{ internship.degree }}</td>
                        <td>@{{ internship.graduation_year }}</td>
                        <td>@{{ internship.internship_type }}</td>
                        <td>@{{ internship.experience }}</td>
                        <td>
                            <a ng-if="internship.resume" href="@{{ internship.resume }}" target="_blank">
                                <i class="fa fa-download"></i> Resume
                            </a>
                        </td>
                        <td>
                            <a ng-if="internship.payment_proof" href="@{{ internship.payment_proof }}" target="_blank">
                                <i class="fa fa-download"></i> Payment Proof
                            </a>
                        </td>
                        <td>
                            <a ng-if="internship.id_proof" href="@{{ internship.id_proof }}" target="_blank">
                                <i class="fa fa-download"></i> ID Proof
                            </a>
                        </td>
                    </tr>
                </tbody>
